...
with timing_container:
    ...
    with snipe_col:
        snipe_seconds_display = st.number_input(
            "Default snipe seconds",
            min_value=1.0,
            max_value=30.0,
            value=float(default_snipe_float),
            step=1.0,
            key="default_snipe_seconds_recovered",
        )
        st.caption("Recovered source fallback for the timing section.")

    st.info(
        "Recovered source note: the original file was truncated near the end. "
        "This timing section was safely closed so the app source remains editable."
    )
